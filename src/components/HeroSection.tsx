  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // نحصل على التفاصيل ونحولها إلى حروف صغيرة لتسهيل البحث
        const details = (result.details || '').toLowerCase();

        // نبحث عن كلمات مفتاحية تدل على أن المحتوى خاص أو غير موجود
        if (details.includes('private') || details.includes('not available') || details.includes('login required') || details.includes('not found')) {
          ...
          // إذا كان خطأ آخر، نعرض الرسالة العامة
    ...
    <section className="w-full">
      <div className="bg-gray-50 py-12 md:py-20">
        <div className="mx-auto max-w-5xl px-4 text-center">
          {/* ... (لا تغيير في هذا الجزء من الكود) ... */}
          <div className="mb-4">
