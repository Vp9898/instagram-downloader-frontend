...
export function HeroSection() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    setIsLoading(true);
    setError(null);
    setData(null); // نبدأ دائمًا من حالة نظيفة
  ...
  // --- دالة جديدة للعودة إلى الفورم الرئيسي ---
  const handleReset = () => {
    setData(null);
    setError(null);
    setUrl('');
  ...
  return (
    <section className="bg-gray-50 py-12 md:py-20 min-h-[calc(100vh-4rem)] flex items-center">
      <div className="mx-auto max-w-5xl px-4 text-center w-full">
        {/* --- منطق عرض جديد ومبسط --- */}
        
        {/* الحالة الأولى: عرض النتائج */}
        {data && !isLoading && (
          <div>
            <button onClick={handleReset} className="mb-8 text-purple-600 hover:text-purple-800 font-semibold">
        ...
        {/* الحالة الثانية: عرض الفورم الرئيسي (مع الخطأ أو بدونه) */}
        {!data && !isLoading && (
          <>
            <div className="mb-4">
            ...
            <div className="mx-auto mt-12 w-full max-w-xl rounded-lg bg-white p-6 shadow-lg sm:p-8">
              <form onSubmit={handleSubmit} className="flex flex-col space-y-4">
                ...
                <input id="url-input" type="text" value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://www.instagram.com/p/..." className="w-full rounded-md border-gray-300 px-4 py-3 text-lg shadow-sm focus:border-purple-500 focus:ring-purple-500" />
                <button type="submit" className="flex w-full items-center justify-center rounded-md bg-gradient-to-r from-pink-500 to-purple-600 px-6 py-4 text-lg font-semibold text-white shadow-md transition-transform duration-200 hover:scale-105 active:scale-95 disabled:opacity-50" disabled={!url}>
        ...
        {/* الحالة الثالثة: عرض مؤشر التحميل (يغطي كل شيء آخر) */}
        {isLoading && (
