...
export function HeroSection() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    setIsLoading(true);
    setError(null);
    // لا تقم بمسح البيانات القديمة هنا، حتى لا تختفي فجأة
    // setData(null); 
    ...
    try {
      ...
      // اقرأ الرد دائمًا، سواء كان ناجحًا أم فاشلاً
      ...
      setData(null); // تأكد من إخفاء أي نتائج قديمة عند حدوث خطأ
  ...
  return (
    <section className="w-full">
      ...
      <div ref={resultsRef} className="w-full bg-white transition-all duration-300">
         {/* أضفنا مسافة padding هنا لتظهر فقط عند وجود نتائج */}
